...
5. 官方公开二进制目前仅提供 Windows 10/11 x64 Lite 桌面安装包，不内置 OCR、Poppler 或 LibreOffice；本地 `jpackage`/WiX、OCR 扩展包及 macOS/Linux 包不属于官方发行版。v0.1.4 尚未做商业代码签名，SmartScreen 可能显示未知发布者；后续广泛分发仍应补签名和可信时间戳。

## OFD

1. 多页 OFD 已完整转换，但跨页视觉连续表格当前分别生成为多张 Word 表格，尚未自动拼成同一张跨页表格。
2. 只识别由水平/垂直矢量线构成的规则表格；无线、嵌套和异形表格尚未实现。
3. `OFD -> PDF` 已按源坐标绘制文字、图片、线条和 PNG/JPEG 等普通图片型签章；签章外观本身为嵌套 OFD 时，当前发行版会返回 `SIGNATURE_APPEARANCE_FAILED` 警告、保留正文并跳过该外观，不会调用未审核的渲染链。二次/三次贝塞尔曲线会折线化。复杂填充、渐变、透明度和裁剪尚未完整支持，弧线暂用端点连线近似，内部 `CM` 变换仍会告警。
...
5. 加密 OFD、私有厂商扩展和部分数字签章外观可能无法解析；系统会失败或给出明确警告。预览到签章图片不代表验证了签名密码学有效性。固定版式 PDF 使用内置字体替代源字体，字形宽度可能存在小幅差异。
