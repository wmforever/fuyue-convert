...
重新执行 `qa-samples/run_qa.py` 后，HTTP 端到端样本 21 条全部严格通过，视觉阈值通过 5 条。`UOF -> DOCX` 已达到规范化文本一致，但仍由 7 页变为 6 页：LibreOffice 将 UOF 独立尾注页合并到了正文末页，因此版式实用性门禁仍不通过，后续继续优化尾注分页。
