...
随后修复可编辑 DOCX 的中文可见性：此前 PDF 的内嵌 CJK 字体名写入 Word 后，本机 LibreOffice 会替换为不含中文字形的字体，导致 XML 字符完整但页面中文为空。现在 PDF/OFD -> DOCX 仅在检测到中日韩文字时嵌入已许可的 Droid Sans Fallback OOXML 混淆字体，并按真实字体高度计算行盒；中文重新可见，密集 50 行页面行距均匀且末行不重叠，仍保持 2210 字符、2 页和 `word/media/` 为零。Java 全量测试 145 项，通过 143 项，失败和错误均为 0，按环境门禁跳过 2 项；HTTP QA 仍为 21/21 严格通过、视觉阈值通过 7。跨 Office 引擎的像素差异率为 `0.0872686624`，主要反映字体栅格化差异，不宣称像素一致。

同一字体嵌入能力已复用于 stable `TXT -> DOCX`：未设置 `FORMAT_CONVERTER_DOCX_CJK_FONT` 且文本确有中日韩字符时嵌入回退字体；显式配置字体时不覆盖用户选择。UTF-16 中文、真实分页和字体部件单测通过，真实 TXT -> DOCX -> PDF 烟测确认此前为空白的中文已正常显示。
