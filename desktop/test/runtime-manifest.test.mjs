...
test('public installer pins its owned directory before recursive uninstall', async () => {
  ...
  assert.match(script, /SetOutPath "\$TEMP"/)
  assert.match(script, /Delete "\$INSTDIR\\Uninstall \$\{PRODUCT_NAME\}\.exe"/)
  assert.match(script, /ClearErrors\s+RMDir \/r "\$INSTDIR"\s+IfErrors 0 uninstall_complete/)
